...
def check(email):
    url = " https://utaswwmbmb.execute-api.ap-south-1.amazonaws.com/login?emailid="+email
...
def register():
    ...
    print(x)
    params = "name="+x[0]+"&emailid="+x[1]+"&phone="+x[2]+"&city="+x[3]+"&infect="+x[4]+"&blood="+x[5]+"&password="+x[6]
    
    if('errorType' in check(x[1])):
        url = "https://cgb1yukks3.execute-api.ap-south-1.amazonaws.com/second?"+params
...
@app.route('/stats')
def stats():
    url = "https://ayvs0vaq2m.execute-api.ap-south-1.amazonaws.com/stage2/getbloodgroupdata"
    response = requests.get(url)
    r = response.json()
    print(r)
...
def requested():
    ...
    print(address)
    url = " https://l38zeue0lj.execute-api.ap-south-1.amazonaws.com/stage1/getdatabybloodgroup?blood="+bloodgrp
    status = requests.request("GET",url)
    a=status.json()
    print(a)
    phone=[]
